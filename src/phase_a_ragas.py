...
def save_phase_a_report(results: list[RagasResult], clusters: dict,
    ...
    if not results:
        print("No results to save — run_ragas_50q() returned empty.")
        return
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    """
    if not results:
        return {"matrix": {}, "dominant_failure_distribution": "", "dominant_failure_metric": "", "insight": ""}

    metrics = ["faithfulness", "answer_relevancy", "context_precision", "context_recall"]
    dists  = ["factual", "multi_hop", "adversarial"]
    matrix = {m: {d: 0 for d in dists} for m in metrics}

    for r in results:
        matrix[r.worst_metric][r.distribution] += 1

    dominant_dist   = max(dists, key=lambda d: sum(matrix[m][d] for m in metrics))
